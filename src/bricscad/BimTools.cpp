...
{
    OperationResult result;
    result.operation = request.kind == TransformKind::Move ? "move" : "rotate";
    result.inputUnits = request.units.empty() ? "mm" : request.units;
    result.availability = availability();
    if (!result.availability.available) {
        addError(result.errors, "bim", "bimUnavailable", result.availability.reason);
        return result;
    ...
    ResolveResult resolved = resolve(database, request.selector, ResolvePurpose::DrawingMutation);
    result.debug = std::move(resolved.debug);
    result.errors = std::move(resolved.errors);
    result.fingerprints = resolved.fingerprints;
    if (!resolved.success) {
        return result;
    }
    ...
    if (!inputScale(database, result.inputUnits, scale, unitError)) {
        addError(result.errors, "units", "unitConversionFailed", unitError);
        return result;
    }

    if (request.kind == TransformKind::Move) {
        if (!finiteVector(request.vector) || request.vector.length() <= kTolerance) {
            addError(result.errors, "vector", "invalidVector", "bim.move braucht einen endlichen, von null verschiedenen Vektor");
            return result;
        }
    } else {
        if (!std::isfinite(request.angleRadians) || std::abs(request.angleRadians) <= kTolerance) {
            addError(result.errors, "angle", "invalidAngle", "bim.rotate braucht einen endlichen Winkel ungleich null");
        }
        if (!finiteVector(request.axis) || request.axis.length() <= kTolerance) {
            addError(result.errors, "axis", "invalidAxis", "bim.rotate braucht eine endliche Rotationsachse ungleich null");
        }
        if (request.baseMode == RotationBaseMode::Explicit
            && (!request.hasExplicitBasePoint || !finitePoint(request.basePoint))) {
            addError(result.errors, "basePoint", "invalidBasePoint", "bim.rotate braucht einen endlichen Basispunkt oder basePointMode");
        }
        if (!result.errors.empty()) return result;
    }

    for (int i = 0; i < resolved.ids.length(); ++i) {
        const AcDbObjectId id = resolved.ids.at(i);
        AcDbEntity* entity = nullptr;
        const Acad::ErrorStatus openStatus = acdbOpenObject(entity, id, AcDb::kForRead);
        if (openStatus != Acad::eOk || entity == nullptr) {
            addError(result.errors, handleText(id), "openFailed", errorStatusText(openStatus));
            continue;
        }
        ObjectData before;
        std::string snapshotError;
        if (!populateObject(id, entity, true, false, before, snapshotError)) {
            addError(result.errors, handleText(id), "snapshotFailed", snapshotError);
        } else {
            result.before.push_back(std::move(before));
        }
        entity->close();
    }
    if (!result.errors.empty()) {
    ...
    AcGePoint3d selectionCenter = AcGePoint3d::kOrigin;
    if (request.kind == TransformKind::Rotate && request.baseMode == RotationBaseMode::SelectionCenter) {
        Bounds combined;
        for (const ObjectData& object : result.before) {
            if (!object.bounds.valid) continue;
            if (!combined.valid) {
                combined = object.bounds;
            } else {
                combined.minPoint.x = std::min(combined.minPoint.x, object.bounds.minPoint.x);
                combined.minPoint.y = std::min(combined.minPoint.y, object.bounds.minPoint.y);
                combined.minPoint.z = std::min(combined.minPoint.z, object.bounds.minPoint.z);
                combined.maxPoint.x = std::max(combined.maxPoint.x, object.bounds.maxPoint.x);
                combined.maxPoint.y = std::max(combined.maxPoint.y, object.bounds.maxPoint.y);
                combined.maxPoint.z = std::max(combined.maxPoint.z, object.bounds.maxPoint.z);
            }
        }
        if (!combined.valid) {
            addError(result.errors, "selectionCenter", "boundsUnavailable", "Auswahlmittelpunkt konnte nicht aus Bounds bestimmt werden");
            return result;
        }
        selectionCenter = boundsCenter(combined);
    }

    AcDbTransactionManager* manager = database == nullptr ? nullptr : database->transactionManager();
    AcTransaction* transaction = manager == nullptr ? nullptr : manager->startTransaction();
    if (transaction == nullptr) {
        addError(result.errors, "transaction", "transactionStartFailed", "BRX-Transaktion konnte nicht gestartet werden");
        return result;
    }
    ...
    for (int i = 0; i < resolved.ids.length(); ++i) {
        ...
        const Acad::ErrorStatus openStatus = transaction->getObject(object, id, AcDb::kForWrite);
        AcDbEntity* entity = openStatus == Acad::eOk ? AcDbEntity::cast(object) : nullptr;
        if (entity == nullptr) {
            addError(result.errors, handleText(id), "writeOpenFailed", errorStatusText(openStatus));
            transactionOk = false;
            break;
        }
        ...
        AcGeMatrix3d matrix;
        if (request.kind == TransformKind::Move) {
            matrix.setToTranslation(request.vector * scale);
        } else {
            AcGePoint3d basePoint = request.basePoint;
            if (request.baseMode == RotationBaseMode::Explicit) {
                basePoint = AcGePoint3d(basePoint.x * scale, basePoint.y * scale, basePoint.z * scale);
            } else if (request.baseMode == RotationBaseMode::SelectionCenter) {
                basePoint = selectionCenter;
            } else {
                const auto before = std::find_if(result.before.begin(), result.before.end(), [&](const ObjectData& value) {
                    return value.id == id;
                });
                if (before == result.before.end() || !before->bounds.valid) {
                    addError(result.errors, handleText(id), "boundsUnavailable", "Entity-Mittelpunkt konnte nicht aus Bounds bestimmt werden");
                    transactionOk = false;
                    break;
                }
                basePoint = boundsCenter(before->bounds);
            }
            matrix.setToRotation(request.angleRadians, request.axis.normal(), basePoint);
        }

        const Acad::ErrorStatus transformStatus = entity->transformBy(matrix);
        result.debug.push_back("transformBy handle=" + handleText(id) + " status=" + errorStatusText(transformStatus));
        if (transformStatus != Acad::eOk) {
            addError(result.errors, handleText(id), "transformFailed", errorStatusText(transformStatus));
            transactionOk = false;
            break;
        }
        ...
        ObjectData after;
        std::string snapshotError;
        if (!populateObject(id, entity, true, false, after, snapshotError)) {
            addError(result.errors, handleText(id), "postTransformSnapshotFailed", snapshotError);
            transactionOk = false;
            break;
        }
        const auto expected = std::find_if(result.fingerprints.begin(), result.fingerprints.end(), [&](const Fingerprint& value) {
            return normalizedHandle(value.handle) == normalizedHandle(after.handle);
        });
        const auto before = std::find_if(result.before.begin(), result.before.end(), [&](const ObjectData& value) {
            return value.id == id;
        });
        if (expected == result.fingerprints.end()
            || before == result.before.end()
            || expected->guid != after.guid
            || expected->bimType != after.bimType
            || before->name != after.name) {
            addError(result.errors, handleText(id), "identityChanged", "Handle, GUID, BIM-Klassifikation oder BIM-Name wurde durch die Transformation veraendert");
            transactionOk = false;
            break;
        }
        result.after.push_back(std::move(after));
    }

    if (!transactionOk) {
        const Acad::ErrorStatus abortStatus = manager->abortTransaction();
        result.debug.push_back("abortTransaction status=" + errorStatusText(abortStatus));
        if (abortStatus != Acad::eOk) {
            addError(result.errors, "transaction", "transactionRollbackFailed",
                "Atomarer Rollback ist fehlgeschlagen: " + errorStatusText(abortStatus));
            result.affectedIds = resolved.ids;
        }
        result.after.clear();
        return result;
    ...
    const Acad::ErrorStatus commitStatus = manager->endTransaction();
    result.debug.push_back("endTransaction status=" + errorStatusText(commitStatus));
    if (commitStatus != Acad::eOk) {
        addError(result.errors, "transaction", "transactionCommitFailed", errorStatusText(commitStatus));
        const Acad::ErrorStatus abortStatus = manager->abortTransaction();
        result.debug.push_back("abortTransaction after commit failure status=" + errorStatusText(abortStatus));
        if (abortStatus != Acad::eOk) {
            addError(result.errors, "transaction", "transactionRollbackFailed",
                "Rollback nach Commit-Fehler ist fehlgeschlagen: " + errorStatusText(abortStatus));
            result.affectedIds = resolved.ids;
        }
        result.after.clear();
        return result;
